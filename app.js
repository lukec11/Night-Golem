...
const hackNightRegex = /next ((?:hr?|cr)(?:e|a|i|o|u)?c?o?w?(?:k|c|p|t|oo?|u(?:t|p)?)(?:e|a)?(?:s|y|lacka)?(?:\s|-)?n(?:e|i|oo?|a)?u?(?:k|g|c|o|t)?k?a?(?:ey|ht|ky|e|t|wu|o(?:p|t)|lacka))/gi;
...
slackEvents.on("message", async (event) => {
  try {
    ...
    } else if (event.text.includes("next hack hack night")) {
      const nextHackNight = nextDate();
      const message = `The next _hack hack night_ is *<!date^${nextHackNight}^{date_short_pretty}|date>*, at *<!date^${nextHackNight}^{time}|time>* local time!`;
